...
class IngresosController extends BaseController
{
        ...
        try {
            ...
            if (isset($_SESSION['sesion_activa']) && $_SESSION['sesion_activa'] === true) {
                ...
                if ($tipo_Movimiento === '2') {
                    ...
                    log_message("debug", "Variables iniciales: idUsuarioCreacion=" . $_SESSION['id_usuario'] . ", correlativo=" . $input['correlativo'] . ", noDocumento=" . $input['noDocumento']);
                    ...
                    log_message("debug", "Datos de productos recibidos: " . print_r($productos, true));
                    ...
                    log_message("debug", "Datos del documento de entrada a insertar: " . print_r($dataDocumentoEntrada, true));
                    log_message("debug", "Datos del documento de salida a insertar: " . print_r($dataDocumentoSalida, true));

                    $id_documento_entrada = $documentosModel->insertDocumento($dataDocumentoEntrada);
                    log_message("debug", "ID del documento de entrada insertado: " . $id_documento_entrada);

                    $id_documento_salida = $documentosModel->insertDocumento($dataDocumentoSalida);
                    log_message("debug", "ID del documento de salida insertado: " . $id_documento_salida);

                    if (!empty($id_documento_entrada) && !empty($id_documento_salida)) {
                        $flagSaveEntrada = $this->guardarProductos($productos, $id_documento_entrada, 2);
                        log_message("debug", "Resultado de guardarProductos en documento de entrada: " . ($flagSaveEntrada ? 'Éxito' : 'Fallo'));

                        $flagSaveSalida = $this->guardarProductos($productos, $id_documento_salida, 6);
                        log_message("debug", "Resultado de guardarProductos en documento de salida: " . ($flagSaveSalida ? 'Éxito' : 'Fallo'));
                    ...
                    $tipo_Movimiento = $input['tipo_Movimiento']; // Asegúrate de que esta línea esté definida y obtenida correctamente
                    ...
                    log_message("debug", "Datos del documento de salida a insertar: " . print_r($commonData, true));
    ...
    public function guardarProductos($productos, $id_documento, $id_tipo_movimiento)
    {
        try {
