...
class EstadoCuentaController extends BaseController
{
    public function estadoDeCuenta()
    {
        ...
        if ($session->get('sesion_activa') === true) {
            ...
            if (str_starts_with($numeroSolicitud, 'V-')) {
                $datosPagos = $estadoDeCuentaModel->obtenerPagosRealizadosSolV($numeroSolicitud, $id_sucursal,$idSolicitud);
            } else {
                $datosPagos = $estadoDeCuentaModel->obtenerPagosRealizados($numeroSolicitud, $id_sucursal,$idSolicitud);
            }

            $datosClientes = $estadoDeCuentaModel->obtenerCliente($numeroSolicitud, $id_sucursal,$idSolicitud);
            $datosSolicitud = $estadoDeCuentaModel->obtenerInfoSolicitud($numeroSolicitud, $id_sucursal, $idSolicitud);
                    ...
                    $productosAnteriores = $db->table('productos_solicitud_anterior')
                        ...
                        ->where('id_solicitud', function ($builder) use ($solicitud) {
                            $builder->select('id_solicitud')
                                ...
                                ->where('id_sucursal', $solicitud->id_sucursal)
                                ->limit(1);
                        })
                        ->get()
                        ->getResultArray();
